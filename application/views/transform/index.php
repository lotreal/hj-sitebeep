...
<div class="container mod_list">
  <form name="myform" id="myform" action="" method="get">
    <div class="operation"><span class="right">
      <input type="text" name="transform" id="keyword" />
      <input type="submit" value="搜索" class="btn"/>
      </span>
	  <?php if($sid):?>
      <a href="<?php echo site_url('transform/add');?>" title="添加转化项目" class="add_btn colorbox">添加</a>
	  <?php else:?>
	  请先添加网站
	  <?php endif;?>
    </div>
    <table class="showlist">
      ...
      <tbody>
        ...
        <tr>
          <td class="w50"><input type="checkbox" name="tid[]" value="1"/></td>
          ...
          <td><a href="<?php echo site_url('transform/update/'.$v['tid']);?>" title="修改" class="colorbox">修改</a> <?php if($v['status']):?><a href="<?php echo site_url('transform/getcode/'.$v['tid']);?>" title="获取代码" class="colorbox">代码</a><?php endif;?> <a href="<?php echo site_url('transform/verify/'.$v['tid'].'/'.$v['status']);;?>" title="审核" class="confirm">审核</a> <a href="<?php echo site_url('transform/delete/'.$v['tid']);;?>" title="删除" class="confirm">删除</a></td>
        </tr>
        <?php endforeach;?>
		<?php else:?>
		<tr>
