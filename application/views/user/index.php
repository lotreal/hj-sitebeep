...
<div class="container mod_list">
  <form name="myform" id="myform" action="" method="get">
    <div class="operation"><span class="right">
      <input type="text" name="username" id="keyword" />
      <input type="submit" value="搜索" class="btn"/>
      </span>
	  <a href="<?php echo site_url('user/add');?>" title="添加新用户" class="add_btn colorbox">添加</a>
    </div>
    <table class="showlist">
      <thead>
        <tr>
          <th class="w50"><a href="javascript:void(0)" id="checkall" title="全选/不选">选择</a></th>
          <th class="wauto">用户名</th>
      ...
      <tbody>
        ...
        <tr>
          <td class="w50"><input type="checkbox" name="uid[]" value="<?php echo $v['uid'];?>"/></td>
          <td>
		  <?php 
			$str = $v['username'];
			$str .= (empty($v['group'])) ? '' : '- '.$v['group'];
			$str .= (empty($v['job'])) ? '' : '['.$v['job'].']';
			echo $str;
		  ?>		  
		  </td>
          ...
          <td><a href="<?php echo site_url('user/update/'.$v['uid']);?>" title="资料" class="colorbox">修改</a> <a href="<?php echo site_url('user/verify/'.$v['uid'].'/'.$v['status']);;?>" title="审核" class="confirm">审核</a> <a href="<?php echo site_url('user/delete/'.$v['uid']);;?>" title="删除" class="confirm">删除</a></td>
        </tr>
        <?php endforeach;?>
		<?php else:?>
		<tr>
